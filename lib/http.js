...
bedrock.events.on('bedrock-express.configure.routes', app => {
  // get root controller for usage endpoints
  const {usage: {rootController}} = bedrock.config['meter-http'];
  const {id: rootControllerId} = rootController;
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const url = `https://${req.get('host')}${req.url}/${meter.id}/usage`;
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      // FIXME: update meter using meters.update()
  ...
  app.post(
    ...
    _createAuthorizeMiddleware({
      ...
      getRootController() {
        // root controller for meter usage is always the metering service
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      // return `id`, `controller`, `sequence`, and `usage` only
      // FIXME: consider returning `usageCapability` to allow for auto-updates
      const {meter: {controller, sequence}} = record;
      res.json({
        id,
        controller,
        sequence,
