...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      meter.id = idEncoder(await idGenerator.generate());
      ...
      */
      ...
      const handlerResult = await HANDLERS.create({meter});
      if(handlerResult && handlerResult.meter) {
        meter = handlerResult.meter;
      }

      // delegate `zMeter` for usage reporting
      const url = `${req.protocol}://${req.get('host')}${req.url}` +
        `/meters/${meter.id}/usage`;
      // FIXME: get appropriate service DID
      const did = 'did:key:mock';
      meter.usageCapability = await ZCAP_CLIENT.delegate({
        url, targetDelegate: did, allowedActions: ['read', 'write']
      });

      // FIXME: insert meter into database
      //meters.insert()
