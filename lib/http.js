...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.get(
    ...
    _authorizeMeterConfigRequest({basePath: routes.basePath}),
    asyncHandler(async (req, res) => {
      // get meter record
...
function _authorizeMeterConfigRequest({basePath}) {
  return _createAuthorizeMiddleware({
    getExpectedTarget({req}) {
      // only support update meter config URL as expected target; controller
      // is the controller of the meter
      const baseUrl = `https://${req.get('host')}${basePath}`;
      const expectedTarget =
...
//   console.log(rootInvocationTarget, 'gets here:_getRootController');
//   const id = rootInvocationTarget.split('/').pop();
  let controller;
  try {
