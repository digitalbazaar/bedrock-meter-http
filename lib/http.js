...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  routes.revocations = `${routes.basePath}/revocations/:revocationId`;
  routes.meterUsageRevocations =
    `${routes.basePath}/:meterId/usage/revocations/:revocationId`;
  ...
  app.post(
    ...
    authorizeZcapInvocation({
      async getExpectedValues() {
        return {
          ...
          // allow list
          rootInvocationTarget: `${config.server.baseUrl}${routes.basePath}`
        };
      },
    ...
    asyncHandler(async (req, res) => {
      ...
      const fullId = `${config.server.baseUrl}${routes.basePath}/${meter.id}`;
